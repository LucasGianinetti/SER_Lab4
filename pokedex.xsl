...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
		...
		<html>
			...
			<body>
				...
				<div class="container-fluid">
					...
					<div id="accordion">
						<!-- https://stackoverflow.com/questions/43424522/xpath-remove-elimiate-duplicates-get-position-of-a-character-check-for-equ -->
						<xsl:variable name="types" select="/pokedex/pokemon/type[not(preceding::type = .)]" /> <!-- ##### A compléter 3 : Ici, vous devez trouver l'expression XPath à mettre dans l'attribut select
	...
	<xsl:template name="lister_pokemon">
		...
		<xsl:param name="filtre"></xsl:param> <!-- ##### A compléter 6 -->
		...
		<div class="col-2 mb-2" role="pokemon">
			...
			<xsl:attribute name="generation">
				...
				<xsl:choose>
					...
					<xsl:when test="id &lt; 252">
						<xsl:value-of select="2"/>
					</xsl:when>
					<xsl:when test="id &lt; 384">
						<xsl:value-of select="3"/>
					</xsl:when>
					<xsl:when test="id &lt; 494">
						<xsl:value-of select="4"/>
					</xsl:when>
					<xsl:when test="id &lt; 650">
						<xsl:value-of select="5"/>
					</xsl:when>
					<xsl:when test="id &lt; 722">
						<xsl:value-of select="6"/>
					</xsl:when>
					<xsl:when test="id &lt; 810">
